...
*   **Light & Dark Mode (Thema's)**: Volledige ondersteuning voor zowel een licht (wit, standaard) als donker (Dracula/slate) thema. Dit is configureerbaar met een URL-parameter en een wisselknop.
    *   **Thema-gevoelige Kleuren & Overrides**: Het kleurenpalet in de WYSIWYG-editor past zich automatisch aan het gekozen thema aan. Gekozen kleuren worden opgeslagen als responsive CSS-variabelen (`var(...)`) zodat gekleurde teksten en codeblokken (zoals `setheading()`) altijd perfect leesbaar blijven en van tint veranderen bij een themaswitch. Het volledig-scherm-sluitlabel past zich ook direct aan het actieve thema aan.
*   **Compacte Modus & Sliding Drawer**: In de compacte modus (`?mode=compact`) bevindt de output zich in een panel dat soepel omhoog schuift. Dit panel past zijn lay-out automatisch aan op basis van het projecttype (Canvas+Console voor Turtle, Preview+Console voor Web, of Console+Tests gestapeld voor console-projecten). De lay-out is responsief en stapelt verticaal op smalle schermen (< 768px). Op mobiele schermen blijft de bovenste knoppenbalk (Run, Valideer, etc.) altijd direct bereikbaar en zichtbaar als de drawer geopend is. Bij het valideren van code of wanneer code tot een fout leidt, schuift de console output automatisch omhoog en opent in de vergrote stand zodat de foutmelding direct en volledig leesbaar is.
*   **Foutregel-Markering (Error Highlighting)**: Bij een syntaxfout of runtimefout wordt de regel in de editor die de fout veroorzaakte direct rood gekleurd met een rode rand aan de linkerkant om de fout snel op te lossen.
*   **Editor Zoom & Volledig Scherm**: Ingebouwde knoppen in de toolbar om het lettertype te vergroten of verkleinen en om de pagina op volledig scherm te tonen. 
    *   **Pauzeren van compacte modus**: Bij het activeren van volledig scherm in compacte modus schakelt de interface automatisch over naar de volledige leerling-layout (editor en output side-by-side).
    *   **Escape-begeleiding**: Een lichte/donkere melding bovenin toont de student dat het volledige scherm gesloten kan worden met de Escape-toets.
    *   **Persistentie**: Uitvoeren en valideren sluiten de fullscreen-weergave niet af.
*   **Interactief Canvas-beheer**: Wanneer de console onder het canvas vergroot of verkleind wordt, schaalt het canvas automatisch in real-time mee om squashing te voorkomen. De zwevende canvas-knoppen (zoom, download, centreren) verplaatsen zich automatisch naar een compacte, horizontale rij als de beschikbare hoogte beperkt is (< 240px).
*   **Diepe Undo-Geschiedenis**: CodeMirror houdt tot 2000 wijzigingsstappen bij in het geheugen voor een diepe en betrouwbare undo-functie (Ctrl-Z).
...
*   **Aan/Uit Toggles**:
    ...
    *   `Lezen` / `Uitvoeren` / `Bewerken` (Sectie-templates): Voeg met één klik een voorgestileerde sectie (kop + genummerde lijst) toe. Staat de cursor al in zo'n sectie? Klik nogmaals op de knop om de complete sectie (inclusief alle stappen, na bevestiging) te verwijderen!
